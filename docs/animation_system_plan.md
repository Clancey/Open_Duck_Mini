...
There is now a **third** instance of the same meta-lesson, and it is worth stating plainly: **twice we diagnosed the wrong layer.** We spent two 300M-step runs shaping head-tracking *reward* when the problem was structural — the head command never reached the reward target — and four more episodic runs interrogating the *reward, phase encoding and terminations* when the problem was the **input data**: a reference motion whose stored velocities contradicted its own pose trajectory ([Phase 6](#phase-6-deferred--df1--episodic-full-body-clips--transitions)). In both cases the training curve looked plausible (a converged 205 in one, a healthy +18.7 start in the other) and the fault sat **upstream of where we were looking** — in the command→reward plumbing, then in the reference file. The rule that ties all three episodes together: **validate your inputs and verify a metric is physically achievable *before* spending GPU on it.** A reference-motion validator that recomputes velocity fields from the pose trajectory would have caught the episodic fault before a single GPU-hour; deriving the S0.1 threshold from the action space would have caught the gate error before two retrains. Both are now stated requirements (the validator in [Phase 1a](#phase-1--core-library--clip-format--1a-test-harness)/R19; the threshold-derivation rule here), because cheap upstream checks repeatedly beat expensive downstream debugging on this project.
...
- **Status (2026-09-02): attempted, root-caused, NOT currently trainable.** A standing full-body wiggle (Disney's approach for animated legs on a balancing biped) was attempted and **failed four times**; the fault was in the **reference motion data**, not the reward/phase/terminations we spent those runs investigating. **This is an enhancement, not a blocker:** the owner already has a *working docked wiggle*. GPU priority has therefore moved to the **postural-emotion standing policy** (below), which yields more per run. Episodic training resumes only once the reference is **regenerated and passes the velocity-consistency validator** (also below).
- **What failed (DONE, FAILED ×4).** Attempts 1–3: OOM, then reward flat at **−540** (worse than initialisation). Attempt 4 (after fixing three paper divergences in `episodic.py`): started healthy at **+18.7** then *declined* 18.7 → 8.6 → 7.9 across 43M steps. Reward *sliding* (not plateauing) was the tell of mutually-contradictory targets, but it took until the fourth failure for anyone to inspect the input clip.
- **Root cause — the reference motion is kinematically self-contradictory** (direct analysis of `standing_wiggle.json`, 150 frames × 59 floats, 50 fps, 3.0 s; layout per [Appendix B](#appendix-b--59-float-authoring-frame-layout)/[D](#appendix-d--episodic-branch-reference)):
  - **Bug 1 — angular-velocity axes transposed.** Deriving `ang_vel` from the stored quaternion trajectory vs the stored `ang_vel` field (indices 32:35): x and z are swapped to three decimals. Mean-absolute discrepancy (x/y/z) = **0.155 / 0.001 / 0.151**. The stored angular velocity describes a *different rotation* than the stored orientation.

    | axis | derived from quaternions | stored in file |
    |---|---|---|
    | x | 0.1264 | **0.3754** |
    | y | 0.0036 | 0.0000 |
    | z | **0.3785** | 0.1260 |

    Prime suspect: the Blender→robot frame conversion `[x,y,z] → [−y,x,z]` was applied to the pose channels but **not** to the derived velocity channels.
  - **Bug 2 — linear velocity zeroed while the root moves.** `root_pos` (0:3) is non-zero, but `lin_vel` (29:32) is **exactly zero on every frame** — same class of fault (velocity fields not derived from the pose trajectory they accompany).
  - **Bug 3 — barely a wiggle.** Peak-to-peak per joint: knees/ankles **0.0044 rad**, hip_pitch 0.014, hip_roll 0.033, hip_yaw 0.064, antennas exactly 0; only the head genuinely moves. It was derived from `dock_wiggle`, authored **deliberately tiny** for a *docked* robot at ×0.5 derating (the dock carries the weight; a docked root does not translate). Both properties were inherited into a *standing* reference where they are wrong.
  - **Why reward declines rather than plateaus.** The imitation reward weights torso position 1.0, linear velocity 1.0 and angular velocity 0.5. The policy is handed contradictory targets — "you are stationary" (pos/linvel) and "you are rotating at 0.375 rad/s about an axis you are not rotating about" (angvel) — which no policy can jointly satisfy, so improving one term degrades another and total reward slides. Compounding it, `w_joint_pos = 15` **punishes the micro-corrections the policy must make to stay balanced** against a near-static reference: it was penalised for balancing while chasing an impossible target.
- **REQUIRED durable mitigation — a reference-motion validator (not optional).** Any 59-float reference **must** have its velocity fields (`lin_vel` 29:32, `ang_vel` 32:35, `joints_vel` 35:51) recomputed from the pose trajectory (`root_pos` 0:3, `root_quat`, `joints_pos` 7:23) and be **rejected** if they disagree beyond tolerance. This would have caught all three bugs before a single GPU-hour was spent, and it protects every future clip. Implemented as part of the Phase 1a asset-validation harness ([Phase 1](#phase-1--core-library--clip-format--1a-test-harness)) and run as a gate before any episodic training run. **See also R19.**
...
- **Tasks:** Use the `episodic` branch (`EpisodicReferenceMotion`, `episodic.py`, `standing.py`) to train per-clip episodic policies; **regenerate and validate the reference first (above)**; **require every clip to start and end inside a validated double-support envelope** (asset validation rejecting endpoints that fail position/velocity/contact checks) and to include an **explicit trained recovery segment**; integrate STAND ↔ EPISODIC_CLIP via the FSM (never a forced kinematic snap).
...
- **Effort:** L. **Dependencies:** Phases 1, 4, 5; a regenerated + validated reference. **Parallelizable:** partially. **Status: BLOCKED on reference regeneration; deprioritised behind the postural-emotion standing policy.**
...
| R19 | **Unvalidated reference motion — kinematically self-contradictory input silently wastes training.** A standing-wiggle reference had its angular-velocity x/z axes transposed and its linear velocity zeroed while the root moved (inherited from a docked clip); the contradictory pos/linvel/angvel targets made episodic reward *decline*, and it cost **four training runs** before anyone inspected the data ([Phase 6](#phase-6-deferred--df1--episodic-full-body-clips--transitions)) | **Realised** | Medium | **Mandatory validator (Phase 1a):** recompute `lin_vel`/`ang_vel`/`joints_vel` from the pose trajectory and **reject** any reference whose stored velocities disagree beyond tolerance, run as a gate before any training run; plus the authoring rule that a clip authored for one mode is not a valid reference for another |
