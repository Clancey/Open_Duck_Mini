...
There is now a **third** instance of the same meta-lesson, and it is worth stating plainly: **we repeatedly diagnosed the wrong layer.** We spent two 300M-step runs shaping head-tracking *reward* when the problem was structural — the head command never reached the reward target — and **six episodic runs** interrogating the *reward, phase encoding, terminations and then the reference file* when there were in fact **two** faults, both upstream of where we were looking: the **input clip was broken** (authored from a *docked* motion, so its root never translates and its legs barely move — nothing full-body to imitate — with a `root_quat` written WXYZ-instead-of-XYZW that renders the robot **upside-down**), and the training stack itself had the PPO actor's action-distribution standard deviation running away (`policy_dist_max_std` to ~28,980) on an unbounded Gaussian policy head ([Phase 6](#phase-6-deferred--df1--episodic-full-body-clips--transitions); full evidence in [`episodic_standing_wiggle_findings.md`](episodic_standing_wiggle_findings.md)). In every case the training curve looked plausible (a converged 205 in one, a healthy +18.7 start in the other) and the fault sat **below where we were looking**. Three rules tie these episodes together: **(a) validate your inputs *before* spending GPU on them** — a kinematic reference validator (`open_duck_anim/reference_validator.py`, now hard-erroring on pinned-root/static-leg standing clips and wired into the authoring path + test suite) would have rejected the clip on day one; **(b) render motion before training on it** — one MuJoCo render showed the feet-in-air/head-in-ground collapse that four rounds of numeric debugging never surfaced; and **(c) instrument the training *internals*, not just the reward — `policy_dist_max_std`, entropy and `v_loss` told the real story, and a declining reward alone sent us chasing the reference file for a fourth run.** *(History note: an earlier version of this section, and R19, first blamed a self-contradictory reference, then over-corrected to "the reference is fully consistent." The settled truth: the reference **is** broken — a real WXYZ/XYZW quaternion defect (renders upside-down, flagged by the validator) **and** a degenerate docked-derived motion with nothing full-body to imitate — but the quaternion defect is **training-irrelevant** (the env's `get_frame` never reads `root_quat`, and `reset` takes base orientation from the model home keyframe, not the file). The clip must be re-authored, not repaired; and the training stack still needs the action-distribution-runaway fix. See Phase 6.)*
...
- **Two earlier hypotheses were tested and FALSIFIED:**
  ...
  - *The reference is kinematically self-contradictory* — **partly, but not in a way that mattered** (this supersedes both a previous "yes, transposed" and a later "no, fully consistent" — the truth is in between). The team validator (`open_duck_anim.reference_validator`) flags the *shipped* `standing_wiggle.json` with 2 errors: `root_quat` was stored **WXYZ** while the 59-float format declares scipy **XYZW**, so re-derived `world_ang_vel` looks x↔z transposed; and `joints_vel` used a central- rather than backward-difference convention. **Both are real** — the user's Bug 1 instinct was correct, and an earlier hand-check that read the quaternion as WXYZ wrongly called it "consistent." **But both are training-irrelevant:** the env's `EpisodicReferenceMotion.get_frame` never reads `root_quat`, `lin_vel=0` is *correct* (the root is exactly static — Bug 2 refuted), and regenerating to XYZW changes the training-consumed `world_ang_vel` by only ≤0.053 rad/s; the 0.4-amplitude run with the same `world_ang_vel` still diverged. The reference has now been **regenerated to pass the validator** (`Clancey/Open_Duck_Playground@episodic`). Its remaining limitation — knees/ankles move only 0.0044 rad, motion is head-led — is a **deliberate balance-safety choice**: the robot has **no torso actuator**, so demanding real trunk rotation forces it to tip (an earlier aggressive reference made the policy learn to *fall*). "Add real torso/hip motion" is therefore counter-indicated on this hardware.
...
- **STILL-USEFUL durable mitigation — a reference-motion validator (now proven).** Recomputing a 59-float reference's velocity fields (`lin_vel` 29:32, `ang_vel` 32:35, `joints_vel` 35:51) from its pose trajectory and **rejecting** disagreement beyond tolerance is cheap insurance for every future clip. It earned its keep here: it **caught the WXYZ/XYZW quaternion-order defect** in the shipped reference that a hand-check missed, and the regenerated reference now passes it (0 errors). Implemented as `open_duck_anim/reference_validator.py` and part of the Phase 1a asset-validation harness ([Phase 1](#phase-1--core-library--clip-format--1a-test-harness)). **See also R19.**
...
| R19 | **Unvalidated reference motion — a bad clip could silently waste training.** *Investigated; partially realised.* The team validator flags the *shipped* `standing_wiggle.json` with two real defects — `root_quat` stored WXYZ while the format declares XYZW (re-derived `world_ang_vel` looks x↔z transposed) and `joints_vel` on a central- vs backward-difference convention. The user's Bug 1 instinct was correct and an earlier "fully consistent" hand-check (which read the quaternion as WXYZ) was wrong. **However these did not cause the four-run failure:** the env's `get_frame` never reads `root_quat`, `lin_vel=0` is correct for a static root (Bug 2 refuted), and the training-consumed `world_ang_vel` shifts ≤0.053 rad/s when fixed — the 0.4-amplitude run with the same profile still diverged. The **realised** fault was an action-distribution runaway (findings doc). Reference regenerated to pass the validator; the validator itself is the durable win. | Guard, partially realised | Low | **Cheap preventive validator (Phase 1a)** — recompute `lin_vel`/`ang_vel`/`joints_vel` from the pose trajectory and **reject** disagreement, run before any training; plus the authoring rule that a clip authored for one mode is not automatically a valid reference for another |
