...
- **Blended/hybrid mode** — animations play on the head/neck/antennas while the RL policy keeps the legs balancing and walking. The animation drives the four head command channels as relative offsets; the policy owns the legs entirely.
...
3. **The deployed policy does not track head commands — measured, not assumed.** Spike S0.1 (executed 2026-09-01 in a validated MuJoCo closed loop) swept `commands[3:7]` against the current `BEST_WALK_ONNX_2.onnx` with the additive head lines removed. **DC gain is ≈0.000–0.012 across all four head channels, standing and walking** — roughly 500× below the 0.6 pass threshold — and sinusoid attenuation is ≈0.00: the policy's own output does not move the head at all. This **corrects the earlier "double-count" framing.** Because the policy contributes ≈0, the additive path at `Open_Duck_Mini_Runtime/scripts/v2_rl_walk_mujoco.py:310-311` is the **sole** source of head motion (effective gain ≈×1), *not* a ×2 double-count. Deleting it today would leave the head motionless; gain-compensating it would halve legitimate motion. The command-channel architecture is retained as the **target** but is now **gated on the Phase 5 leg/neck reward split**, which moves onto the critical path; the additive line is deleted in the *same change* that ships the retrained ONNX. A second, high-severity S0.1 finding: in additive mode, **step inputs at `neck_pitch`/`head_yaw` range extremes topple the robot** (tilt ≈179°, z<0) in both stand and walk, so interim head animation must be amplitude- and slew-limited (D13). See [§3.4 D1/D13](#34-confirmed-defects-and-gaps), [Phase 0/S0.1](#phase-0--de-risking-spikes-gates-run-first) and [risk R11/R16](#8-risk-register).

The single highest-leverage training change is restoring Disney's leg/neck reward split (neck gets ~6.7× the leg position weight); Open Duck ported the table but collapsed the buckets and then deleted the neck entries (`custom_rewards.py:78-82`). Per S0.1 this reward split is now on the **critical path for the hybrid feature**, not an optimisation: it is the only route to making the policy track head commands, and the additive path can be removed only once it lands.
...
- G3. A blended mode that animates head/neck/antennas while the RL policy walks/stands, using the command channel as the injection port.
...
- G7. Training upgrades: leg/neck reward split, retrain, ONNX export.
...
| `apirrone/Open_Duck_Mini_Runtime` | branch `v2` | On-robot runtime (Pi Zero 2W) | Injection port, additive-path interim + removal, FSM |
...
|---|---|---|---|---|
| D1 | **Head-command model correction (was "double-count").** S0.1 measured the policy's own head-command gain at ≈0, so the additive head path is the **sole** source of head motion, *not* a ×2 double-count | `v2_rl_walk_mujoco.py:310-311` | Deleting it today ⇒ head motionless; halving/gain-compensating ⇒ ≈0.5 gain | **Keep the additive path as an amplitude/slew-limited, load-bearing interim; delete it only in the same change as the Phase 5 retrained ONNX** |
...
| D5 | Neck/head/antennas **discarded from imitation reward** | `custom_rewards.py:78-82` | **Confirmed by S0.1 as the root cause** of zero head-command-following; blocks the command-channel architecture | Restore leg/neck split (Phase 5 — now on the critical path) |
...
| D13 | **Additive-mode toppling at range extremes (S0.1 safety finding).** Step inputs at `neck_pitch`/`head_yaw` extremes fall the robot (tilt ≈179°, z<0) in stand and walk; `policy_only` perturbs legs ≤0.02 rad and never falls | `v2_rl_walk_mujoco.py:310-311` + additive interim | Current shipping behaviour is not balance-safe at full authored deflection with steps | Derive an empirical safe envelope (max deflection + max command slew per channel) before any hardware demo; enforce in `open_duck_anim` limits (Phase 3/4) |
...
- **A. Injection port = the command channel (target; gated on Phase 5).** `commands[3:7]` (neck_pitch, head_pitch, head_yaw, head_roll) is trained over its full range and already in the deployed observation, so it is the intended long-term injection port, driving those four channels as **relative offsets from nominal** (Disney's Δh_head / Δθ_head; transform in [§6.3](#63-absolute-pose-to-relative-command-transform)). **Measured reality (S0.1):** the *current* ONNX does not follow these commands (DC gain ≈0 on all four channels, stand and walk), so this architecture is **not reachable until the Phase 5 leg/neck reward split** teaches command-following. **Interim (Phases 3–4):** head motion is produced via the existing additive path at `:310-311` — which S0.1 shows is the *sole* source of head motion, not a double-count — kept **amplitude- and slew-limited** to a validated safe envelope (D13) so it cannot topple the robot. The additive line is removed in the same change that ships the retrained ONNX (Phase 5).
...
- **G. Training reward split** (leg vs neck buckets) — also the expected route to removing the additive head path safely.
...
The animation engine is a pre-policy stage that produces (i) the four head command offsets injected into `commands[3:7]`, and (ii) direct show-function outputs (antennas, eyes, sound, projector) that bypass the policy. In dock mode it additionally produces direct leg/head joint targets, bypassing the policy entirely.
...
flowchart TD
    ...
    subgraph Training[Open_Duck_Playground]
      REF --> IMIT[Imitation reward<br/>leg/neck split]
    ...
    subgraph Runtime[Pi Zero 2W @ 50 Hz]
      ...
      ENG -->|head offsets commands 3:7| OBS[Observation build 101]
      ONNX --> POL[Policy]
      OBS --> POL
...
**Interim wiring (pre-Phase 5).** Because the current policy does not track `commands[3:7]` (S0.1: DC gain ≈0), until the reward split lands the `animation_delta` is applied through the **additive head path** (`v2_rl_walk_mujoco.py:310-311`) — the sole source of head motion — rather than through the command channel. The `open_duck_anim` transform above is therefore **staged**: it computes and clamps the same delta, but the runtime routes it additively (further limited to the D13 safe operating envelope — max deflection and max slew per channel) in the interim, and switches to the command channel — deleting the additive line — when the Phase 5 ONNX ships. The training-range and `jnt_range` clamps apply in both wirings.
...
def evaluate(self, t_now: float, mode: Mode, trigger_snapshot: Triggers) -> EngineOutput:
    ...
    Returns:
      EngineOutput(
        head_command_offsets: np.ndarray[4],       # added to commands[3:7], see 6.3
...
The plan front-loads **risk** (three spikes first), then the dock demo (fastest visible hardware result). Spike S0.1 has already run and its FAIL result **promotes the training reward split (Phase 5) onto the critical path** for the hybrid feature; only the deferred full-body work (Phase 6) remains off the main line. Acceptance criteria are stated as objective thresholds (proposed; tune on hardware) rather than adjectives.
...
- **Acceptance:** each spike returns a documented pass/fail against its threshold. **S0.1: DONE — FAILED; its result is folded into Decisions A/§6.3, Phases 4 and 5, and risks R11/R16.** S0.2 and S0.3 pending.
...
### Phase 4 — Runtime integration + interim hybrid + FSM

- **Objective:** Blended head animation while walking/standing via the **amplitude/slew-limited interim additive path** (goals G3, G6; decisions A, F). **S0.1 result is in: the additive path is retained; its removal is deferred to Phase 5.**
- **Tasks:** Route engine head offsets through the existing additive path (`:310-311`) — the sole source of head motion per S0.1 — while **enforcing the D13 safe operating envelope** (per-channel max deflection + max command slew rate, derived empirically before any hardware demo) so no input can reach the toppling extremes; feed the offset every control tick (bypassing 20 Hz quantisation, §6.1); implement the mode FSM with quantitative guards, `ARMING` and `FAULT` ([§4.3](#43-mode-fsm)); prefer the single STAND+WALK locomotion policy to avoid handoff; reconcile the 67-vs-101 obs-length divergence (D9) and assert obs length against the ONNX input at startup; keep leg channels discarded under policy. **Does not delete the additive line** — that ships coupled with the Phase 5 retrain.
- **Repos/files:** `Open_Duck_Mini_Runtime` (`v2_rl_walk_mujoco.py`, `xbox_controller.py`), imports `open_duck_anim`.
- **Acceptance:** an authored head-yaw step *within the safe envelope* produces the intended deflection (interim additive gain ≈1.0 per S0.1) with **no toppling — IMU tilt ≤ 0.15 rad and zero falls over 10 one-minute trials** at maximum authored deflection and slew; **RMS head tracking error vs authored reference ≤ 0.1 rad** and phase lag ≤ 60 ms at 1 Hz; FSM transitions fire only when guards G hold; startup always passes through `ARMING`; the safe envelope is enforced in `open_duck_anim` limits and unit-tested; a runtime assertion prevents the command-channel and additive injections from being active simultaneously.
- **Effort:** M. **Dependencies:** Phase 1; S0.1 (done); ideally Phase 3. **Parallelizable:** partially.

### Phase 5 — Training reward split + retrain + ONNX (critical path)

- **Objective:** Restore Disney's leg/neck reward split; retrain; export (goal G7; decision G; D5, D6). **Per S0.1 this is on the critical path for the hybrid feature, not an optimisation:** it is the only route to making the policy actually track head commands (current gain ≈0), and it is what finally allows the additive path to be deleted.
- **Tasks:** In `custom_rewards.py`, split `reward_imitation` into leg/neck buckets (`w_joint_pos_leg=15.0`, `w_joint_pos_neck≈100.0`, `w_joint_vel_leg=1e-3`, `w_joint_vel_neck≈1.0`, separate action-rate leg 1.5/neck 5.0, action-accel leg 0.45/neck 5.0); stop discarding indices 5:9 (`:78-82`); do NOT reintroduce the broken `root_quat_slice_start=3` term (D6); retrain via MJX/Brax PPO; export ONNX (opset 11) and validate obs 101 / action 14 unchanged; **and, coupled in the same change, delete the additive head lines (`:310-311`) and switch head injection to the command channel ([§6.3](#63-absolute-pose-to-relative-command-transform)).**
- **Repos/files:** `Open_Duck_Playground` (`custom_rewards.py`, `joystick.py`, `export_onnx.py`); new checkpoint into this repo.
- **Acceptance:** **re-run Spike S0.1 against the retrained policy on the same harness and require per-channel DC gain ≥ 0.6 and cross-coupling ≤ 0.2, standing and walking** (the current policy scored ≈0); **in-sim RMS head-tracking error reduced ≥ 2× vs the current checkpoint**; ONNX loads with input `obs[1,101]`, output `continuous_actions[1,14]`; the additive-line deletion ships together with this checkpoint (never before).
- **Effort:** L (compute-bound). **Dependencies:** Phase 4 for on-hardware validation; training itself can start earlier. **Parallelizable:** yes (offline).
...
| R8 | Additive head path mis-handled (D1) — deleted too early (⇒ motionless head) or left in after Phase 5 (⇒ a *real* double-count once following is trained) | Medium | High | Keep as a documented amplitude/slew-limited interim; gate deletion on the S0.1 re-measure; delete it in the same change as the Phase 5 ONNX; assert on startup that the additive interim and command-channel injection are mutually exclusive |
...
| R11 | **REALISED (S0.1, 2026-09-01): the deployed policy does not follow head commands** — DC gain ≈0 on all four channels, stand and walk, vs a 0.6 threshold; the imitation reward excludes head/neck (D5) | Realised | High | Staged approach: retain the additive path (sole source of head motion) as an **amplitude/slew-limited interim**; the Phase 5 reward split restores following; re-measure S0.1 (gain ≥ 0.6, cross ≤ 0.2) as the gate to delete the additive line, shipped with the retrained ONNX |
...
| R16 | **REALISED (S0.1, D13): additive head animation topples the robot at large deflections** — step inputs at `neck_pitch`/`head_yaw` extremes fall it in stand and walk (tilt ≈179°, z<0) | Realised | High | Derive an empirical safe operating envelope (max deflection + max command slew per channel) before any hardware demo; enforce in `open_duck_anim` limits; never drive the interim additive path to range extremes with steps (Phase 3/4) |
...
3. **Compute for retraining — ANSWERED: GPU available; access pending key authorisation.** The owner has an RTX 4090 workstation and a Docker host with an RTX 3090 (`tower.local`). SSH key access to `tower.local` was not yet authorised at time of writing, so Phase 5/6 MJX PPO runs are unblocked on hardware but pending key setup. Now higher priority since S0.1 put Phase 5 on the critical path.
