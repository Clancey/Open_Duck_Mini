...
3. **There is an active double-count bug in the runtime head path.** `Open_Duck_Mini_Runtime/scripts/v2_rl_walk_mujoco.py:310-311` feeds the head command to the policy via the observation *and* adds it on top of the policy output, roughly doubling authored head motion. This line is active (not commented). **Crucially, whether it is safe to simply delete is unproven** — randomising head commands in the observation during training does not by itself teach the policy to *follow* them, and the walking imitation reward excludes head/neck entirely. Removing the additive path could yield weak or zero head motion. A **Phase 0 spike (S0.1)** must characterise the current ONNX's head-command response before this decision is made; until then the additive path stays as a documented, gain-scaled interim (see [Phase 4](#7-phased-delivery-plan) and [risk R11](#8-risk-register)).

The single highest-leverage training change is restoring Disney's leg/neck reward split (neck gets ~6.7× the leg position weight); Open Duck ported the table but collapsed the buckets and then deleted the neck entries (`custom_rewards.py:78-82`). This is also what is expected to make head-command-following strong enough to remove the additive path safely.
...
| `apirrone/Open_Duck_Mini_Runtime` | branch `v2` | On-robot runtime (Pi Zero 2W) | Injection port, double-count fix, FSM |
...
|---|---|---|---|---|
| D1 | **Double-count**: head command fed to policy *and* added to output | `v2_rl_walk_mujoco.py:310-311` | Authored head motion ~doubled | **Do not delete blindly.** Characterise response (Spike S0.1); resolve in Phase 4 conditional on that outcome |
...
| D5 | Neck/head/antennas **discarded from imitation reward** | `custom_rewards.py:78-82` | Head motion under-tracked; likely root cause of weak command-following | Restore leg/neck split (Phase 5) |
...
- **A. Injection port = the command channel.** `commands[3:7]` (neck_pitch, head_pitch, head_yaw, head_roll) is already trained over its full range and already in the deployed observation. The animation engine drives those four channels as **relative offsets from nominal**, exactly like Disney's Δh_head / Δθ_head (transform defined in [§6.3](#63-absolute-pose-to-relative-command-transform)). **Contingency:** whether the additive path at `:310-311` can be removed depends on Spike S0.1's measured head-command response of the *current* ONNX. If response is inadequate, the additive path is retained as a documented, gain-scaled interim (double-count compensated) until the reward split (Phase 5) lands. Phase 4's approach is conditional on that outcome.
...
The plan front-loads **risk** (three spikes first), then the dock demo (fastest visible hardware result), and defers retraining and full-body work. Acceptance criteria are stated as objective thresholds (proposed; tune on hardware) rather than adjectives.
...
- **S0.1 — Current-ONNX head command response characterisation (gates the D1 decision).** In MuJoCo, with the additive lines `:310-311` removed, sweep `commands[3:7]` across the training ranges against the *current* `BEST_WALK_ONNX_2.onnx` and measure head-joint response: **DC gain** (Δjoint/Δcommand per channel), **lag** (phase at a reference frequency), and **cross-coupling** (off-diagonal response). **Pass threshold (proposed):** per-channel gain ≥ 0.6 and cross-coupling ≤ 0.2 of the diagonal. **Contingency:** if it fails, the additive path stays as a documented, gain-scaled interim (compensate the double-count by halving, or remove the command from one of the two paths) until Phase 5 lands. Outcome is recorded and drives Phase 4.
...
- **Repos/files:** throwaway scripts in this repo + `Open_Duck_Mini_Runtime`; MuJoCo model.
- **Acceptance:** each spike returns a documented pass/fail against its threshold; S0.1's result is written into Phase 4's plan.
- **Effort:** S each. **Dependencies:** S0.2 needs a Phase 1 prototype. **Parallelizable:** S0.1 and S0.3 immediately; S0.2 after Phase 1 prototype.
...
### Phase 4 — Runtime integration + double-count resolution + FSM

- **Objective:** Blended head animation while walking/standing (goals G3, G6; decisions A, F). **Approach conditional on Spike S0.1.**
- **Tasks:** Apply the S0.1 outcome — if head response passes, remove the additive path (`:310-311`) and feed head purely via the observation; if it fails, keep the gain-scaled interim and document it. Route engine head offsets into `commands[3:7]` every control tick (bypassing 20 Hz quantisation, §6.1); implement the mode FSM with the quantitative guards, `ARMING`, and `FAULT` ([§4.3](#43-mode-fsm)); prefer the single STAND+WALK locomotion policy to avoid handoff; reconcile the 67-vs-101 obs-length divergence (D9) and assert obs length against the ONNX input at startup; keep leg channels discarded under policy.
- **Repos/files:** `Open_Duck_Mini_Runtime` (`v2_rl_walk_mujoco.py`, `xbox_controller.py`), imports `open_duck_anim`.
- **Acceptance:** with the additive path removed (or interim gain applied), an authored 0.5 rad yaw produces **0.5 ± 0.1 rad** measured head yaw (not ~1.0); **RMS head tracking error vs authored reference ≤ 0.1 rad** and phase lag ≤ 60 ms at 1 Hz; the duck walks/stands with **IMU tilt ≤ 0.15 rad and zero falls over 10 one-minute trials**; FSM transitions fire only when guards G hold; startup always passes through `ARMING`.
- **Effort:** M. **Dependencies:** Phase 1; S0.1; ideally Phase 3. **Parallelizable:** partially.

### Phase 5 — Training reward split + retrain + ONNX

- **Objective:** Restore Disney's leg/neck reward split; retrain; export (goal G7; decision G; D5, D6). Expected to make head-following strong enough to retire any Phase 4 interim additive path.
- **Tasks:** In `custom_rewards.py`, split `reward_imitation` into leg/neck buckets (`w_joint_pos_leg=15.0`, `w_joint_pos_neck≈100.0`, `w_joint_vel_leg=1e-3`, `w_joint_vel_neck≈1.0`, separate action-rate leg 1.5/neck 5.0, action-accel leg 0.45/neck 5.0); stop discarding indices 5:9 (`:78-82`); do NOT reintroduce the broken `root_quat_slice_start=3` term (D6); retrain via MJX/Brax PPO; export ONNX (opset 11) and validate obs 101 / action 14 unchanged.
- **Repos/files:** `Open_Duck_Playground` (`custom_rewards.py`, `joystick.py`, `export_onnx.py`); new checkpoint into this repo.
- **Acceptance:** **in-sim RMS head-tracking error reduced ≥ 2× vs the current checkpoint**; ONNX loads with input `obs[1,101]`, output `continuous_actions[1,14]`; on-hardware S0.1 gain re-measured ≥ 0.9 per channel, enabling additive-path removal.
...
| R8 | Double-count regression reappears (D1) | Low | High | Resolve per S0.1; add a runtime assertion/test that head command is not additively re-applied unless the interim gain flag is set |
...
| R11 | **Head-command-following is weak/absent** — training randomised commands but the imitation reward excludes head/neck (D5); removing the additive path yields little motion | High | High | Spike S0.1 gates the decision; keep gain-scaled interim additive path until the Phase 5 reward split restores following; re-measure gain before removal |
...
3. **Compute for retraining.** Is a GPU/cloud budget available for Phase 5/6 MJX PPO runs, or is retraining local-only? This sets the timeline for the reward split.
